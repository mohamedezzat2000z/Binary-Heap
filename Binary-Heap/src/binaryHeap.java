...
public class binaryHeap {
    ...
    public void bulidMaxHeap(int[] nums) {
        ...
        for (i = (x / 2); i >= 0; i--) {// loop over the upper part have of the array from the lowest element
            heapify(nums, i, x);// allocate downwards
        }
    }
    ...
    public void sort(int[] array) {
        ...
        for (i = 1; i < x; i++) {// loop over the entered array
            ...
            heapify(array, 0, x - i);// allocate the element down
    ...
    public void heapify(int[] nums, int k, int len) {
        ...
        int hole = k;// get the current place of the element to allocate down from this place
        while (hole * 2 + 1 < len) {// check if the node did not reach the root
            int left = hole * 2 + 1;// get the left element
            int right = left + 1;// get the right element
            int target;// set the variable for the next target for the element ot go to
            if (right < len && (nums[right] > nums[left])) {// check if the right exist and is larget than the left next
                                                            // element
                target = right;// the next target right
            } else {
                target = left;// the next target left
            }

            if (nums[hole] < nums[target]) {// if the target element is bigger than the hole
                // swaping steps
                int s = nums[hole];
                ...
                break;// break if the element is larger then its children
            }
        }
    }
}
